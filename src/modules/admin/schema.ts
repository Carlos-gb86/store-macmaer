...
export function productInput(product: Product): ProductInput {
  return {
    ...product,
    currency: "SEK",
    tax_category_key: product.tax_category_key ?? "standard_goods",
    shipping_class_key: product.shipping_class_key ?? "standard",
...
export const adminTagSchema = z.object({
  id: z.uuid(),
  name,
  name_sv: z.string().trim().max(200).nullable().default(null),
  slug,
