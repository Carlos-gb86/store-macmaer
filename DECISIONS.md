...
Legacy WooCommerce migration is review-first. The preparation command consumes REST JSON exports and generates a catalogue mapping manifest, redirect candidates, and idempotent SQL for reviews. It performs no network or database writes. Product configurations remain flagged for manual mapping because WooCommerce add-ons and variations cannot be safely flattened into Macmaer's generic option model without owner review. Imported reviews remain pending until individually approved.
