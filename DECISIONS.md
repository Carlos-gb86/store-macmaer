...
## Phase 7 — reviews, SEO, and migration — 2026-09-12

Public product reviews are service-boundary writes with a honeypot, bounded submissions, and HMAC-only email identity storage. A paid order for the same product and normalized email marks the review as verified, but does not bypass moderation. Anonymous database roles can read only approved reviews for active products and active testimonials; they cannot insert reviews or inspect the redirect audit table. Homepage testimonials are editable copies so editorial shortening never mutates a customer's original words.

SEO output uses the configured canonical site origin and includes canonical/Open Graph metadata, sitemap and robots routes, plus escaped Organization, WebSite, Product, review, aggregate-rating, and breadcrumb JSON-LD. Indexing remains fail-closed behind `SEO_INDEXING_ENABLED`. It must stay false until `macmaer.com` points to the new application, `NEXT_PUBLIC_SITE_URL` uses that canonical domain, final content is approved, and the legacy redirect map is reviewed.

Legacy WooCommerce migration is review-first. The original offline preparation command remains useful for redirects, while the replacement local pipeline uses read-only REST credentials and GET requests to import the complete published catalogue. WooCommerce variation attributes map to true variant axes/SKU variants; supported Product Add-Ons map to separate configurable options. Unknown or unsupported plug-in metadata is reported rather than guessed or discarded.

An attribute is a true variant axis only when every source variation selects an exact value. Wildcard attributes remain ordinary configurable options, while their WooCommerce variation/SKU records are preserved separately for draft review. Three known WCPA scrunchie packs (`3545`, `3557`, `3561`) are explicit exceptions: each maps by legacy ID to one five-position repeated colour option with duplicates allowed, fixed pricing, and no variant/stock combinations. Unknown WCPA form IDs remain report-only because their definitions require an administrator/nonce endpoint.

Imported products always remain drafts for catalogue review. Approved source reviews retain approved status but never carry source email addresses. Media is downloaded, decoded, normalized and copied to content-addressed Supabase Storage paths; identical normalized bytes share one object per draft/public bucket, while every legitimate product/variation placement and legacy media ID remains distinct. Source URLs are not stored in catalogue image records. Unique legacy IDs and a service-role-only atomic product-graph function make reruns duplicate-safe. Historical customers and orders remain explicitly excluded.

## Phase 8 — production hardening — 2026-09-13

The stable Vercel production alias is the acceptance origin while the existing WordPress shop remains live. Search indexing stays fail-closed. The intended future canonical storefront is `macmaer.se`; domain connection, Stripe payment-domain registration, URL migration, and enabling indexing remain late launch gates and do not block hardening work.

All responses receive a static Content Security Policy and defense-in-depth browser headers. The policy retains static rendering compatibility, permits the exact configured Supabase origin plus Stripe Elements/Link surfaces, blocks embedding and plug-ins, and avoids cross-origin isolation because Stripe does not support it. Stripe publishable and secret keys must declare the same test/live mode whenever checkout is enabled, webhook secrets must use the signing-secret format, and indexed origins must use HTTPS.

Next.js instrumentation emits a minimal structured correlation record for unhandled server errors into Vercel Runtime Logs. It deliberately excludes error messages, headers, bodies, and query strings because they may contain customer information or payment secrets. A root-level error boundary gives customers an accessible recovery action even if the root layout fails. No additional monitoring vendor or customer-data processor is introduced; Vercel Observability remains the initial monitoring surface.

Contact and review rate-limit checks now execute in the same database transaction as their inserts, serialized by privacy-preserving HMAC identity. The callable functions are denied to anonymous and ordinary authenticated roles and available only through the server service credential. Public form errors are programmatically associated with their fields, and the mini-cart moves focus into its disclosure and restores it on keyboard dismissal.

`/admin/settings/readiness` is the operational source for non-secret runtime and commerce launch checks. It reports feature gates, Stripe key mode, signed-event history, catalogue completeness, shipping coverage and the Sweden-only free threshold, tax review state, currency fallback/freshness, discount configuration, policy snapshots, email failures, backups, and final acceptance work. It does not replace owner/accountant/provider review. The detailed incident, backup, rollback, and cutover sequence lives in `docs/PRODUCTION_RUNBOOK.md`.

## Bilingual storefront and catalogue refinement — 2026-09-14

The customer storefront supports English and Swedish without changing established product, checkout, canonical, or legacy-redirect URLs. A secure one-year locale cookie persists an explicit header selection; otherwise the browser's accepted language selects Swedish only for Swedish-speaking visitors, with English as the default. Storefront output is already request-specific, so this does not weaken an existing static-rendering contract. The selected locale is applied to visible copy, form validation, checkout status, transactional action feedback, customer policy text, accessible labels, and page metadata. Admin remains English.

English catalogue content is canonical and required. Nullable Swedish fields cover products, collections, tags, options, values, variants, image alt text, SEO copy, and managed homepage/About content. Storefront localization happens after the cached language-neutral catalogue read, and each missing Swedish value falls back independently to English. Stable slugs, identifiers, SKUs, configuration keys, prices, tax/shipping configuration, and immutable financial snapshots are not translated. Separate locale-prefixed URLs and independently indexable Swedish pages remain a future SEO choice rather than being introduced implicitly.

The top navigation is sticky, uses larger menu text, and replaces its former shop shortcut icon with the locale control. The story moves from the homepage to `/about`; its managed image and lead copy remain editable through the existing homepage-content record. Shop search is debounced and select filters update immediately through URL query parameters, preserving shareable/filterable results without an apply button. Customer-facing diagonal arrows use one SVG icon component, and product tags use a searchable badge selector in admin.

Customer-facing dropdowns use the accessible Radix Select primitive with Macmaer's own trigger, menu, focus, and selected-state styling; native selects remain in admin where compact data-entry behavior is more important. Country and locale choices include decorative flag cues, while text labels remain the accessible names. Non-hero storefront headings use a quieter responsive scale, with the homepage hero retaining its intended visual emphasis.
